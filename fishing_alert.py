...
def fetch_openmeteo():
    ...
    url   = (
        ...
        "&hourly=sea_surface_temperature,wind_speed_10m,wave_height,wave_period,pressure_msl"
...
# -------- MAIN --------
def main():
    # Load state (avoid duplicate alerts per day)
    ...
    for day in {now.date(), (now+dt.timedelta(days=1)).date()}:
        ...
        for fw_start, fw_end in flood_windows:
            if fw_start.date() != day: continue
            label = None
            if overlaps(fw_start, fw_end, *dawn): label = "dawn"
            if overlaps(fw_start, fw_end, *dusk): label = "dusk" if label is None else label
            if not label: continue
            if fw_end < now or fw_start > now+dt.timedelta(hours=24): continue
    ...
    for start,end,label in windows:
        ...
        scored.append({
            ...
            "sst": val_sst, "wind_kt": wind_kt, "wave_m": val_wave, "wavep": val_wavep,
...
f"Wind/Swell/Pressure: SW {rec['wind_kt']:.0f} kt, {rec['wave_m']:.1f} m @ ~{rec['wavep']:.0f} s, pressure trend factored\n"
...
f"Tip: {tip}"
    ...
    if pick_green and not sent_green_today:
        send_push(pick_green,"GREEN"); state.setdefault(today,{})["green"]=True; updated=True
    if pick_amber and not sent_amber_today:
        send_push(pick_amber,"AMBER"); state.setdefault(today,{})["amber"]=True; updated=True
